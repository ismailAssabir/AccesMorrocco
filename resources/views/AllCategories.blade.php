            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm" id="categories-table">
                    ...
                    <tbody class="divide-y divide-slate-50" id="categories-tbody">
                        ...
                        data-id="{{ $category->id }}" 
                        data-nom="{{ \Illuminate\Support\Str::lower($category->nom) }}" 
                        data-desc="{{ \Illuminate\Support\Str::lower($category->desc ?? '') }}" 
                        data-created="{{ $category->created_at ? $category->created_at->timestamp : 0 }}">                            
                                ...
                                <div class="flex items-center gap-3">
                                    <div class="w-8 h-8 rounded-xl bg-[#b11d40]/10 flex items-center justify-center flex-shrink-0">
                                        <span class="text-[#b11d40] font-black text-xs">
                                            {{ strtoupper(substr($category->nom, 0, 2)) }}
                                        </span>
                                    </div>
                                    <p class="font-bold text-slate-800">{{ $category->nom }}</p>
                                </div>
                            </td>
                            ...
                            <td class="px-4 py-4">
                                <div class="flex items-center gap-1">
                                    ...
                                    @can('category.delete')
                                    <button type="button" 
                                            onclick="openGlobalDeleteModal('{{ route('categories.destroy', $category) }}', 'Supprimer la catégorie', 'Êtes-vous sûr de vouloir supprimer cette catégorie ? Cette action est irréversible.')"
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea name="desc" rows="4" placeholder="Description de la catégorie..."
                                      class="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-700 focus:outline-none focus:border-[#b11d40] focus:ring-1 focus:ring-[#b11d40] resize-none">{{ old('desc') }}</textarea>
                        </div>
                    </div>
