<x-app-layout>
    <div class="p-8 bg-[#F8FAFC] min-h-screen font-sans text-slate-900">
        ...
        <div class="bg-white border border-slate-200 rounded-2xl shadow-sm p-4 mb-6 flex flex-wrap items-center gap-4">
            ...
            <div class="bg-slate-50 p-1 rounded-xl border border-slate-200 shadow-sm flex items-center shrink-0">
                <button type="button" data-period="monthly" class="period-toggle px-5 py-2 rounded-lg text-[10px] font-black uppercase transition-all {{ $period == 'monthly' ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-400 hover:bg-slate-100 hover:text-slate-600' }}">Mensuel</button>
                <button type="button" data-period="today" class="period-toggle px-5 py-2 rounded-lg text-[10px] font-black uppercase transition-all {{ $period == 'today' ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-400 hover:bg-slate-100 hover:text-slate-600' }}">Aujourd'hui</button>
            </div>
        </div>

        {{-- ═══════════ GRID OF CARDS ═══════════ --}}
        <div id="dept-grid-container">
            @include('departements.partials.cards')
        </div>
    </div>
    ...
    <script>
        ...
        function fetchDepartements() {
            ...
            // 1. Fetch HTML Cards
            fetch(`{{ route('departements.index') }}?period=${currentPeriod}&search=${encodeURIComponent(search)}&get_cards=1`, {
            ...
            // 2. Fetch JSON KPIs
            fetch(`{{ route('departements.index') }}?period=${currentPeriod}&search=${encodeURIComponent(search)}`, {
                headers: { 'X-Requested-With': 'XMLHttpRequest' }
            })
            .then(res => res.json())
            .then(data => {
