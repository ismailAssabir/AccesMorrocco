<x-app-layout>
    <div class="px-8 py-8 min-h-screen bg-[#F8FAFC]">
        ...
        <div class="mb-8">
            ...
            <h1 class="text-2xl font-extrabold tracking-tight text-slate-800">Modifier la Réunion</h1>
            <p class="text-sm font-medium text-slate-500 mt-1">Mettez à jour les informations de la réunion.</p>
        </div>
        ...
        <div class="max-w-3xl bg-white border border-slate-200 rounded-3xl shadow-sm overflow-hidden">
            ...
            <form action="{{ url('/reunions/edit/' . $reunion->idReunion) }}" method="POST" class="p-8">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="titre" value="{{ old('titre', $reunion->titre) }}" required class="w-full bg-slate-50 border border-slate-200 focus:border-indigo-500 focus:bg-white rounded-xl px-4 py-3 text-sm outline-none transition-all">
                    </div>
                    ...
                    <div>
                        ...
                        <select name="type" required class="w-full bg-slate-50 border border-slate-200 focus:border-indigo-500 focus:bg-white rounded-xl px-4 py-3 text-sm outline-none transition-all">
                            <option value="Interne" {{ old('type', $reunion->type) == 'Interne' ? 'selected' : '' }}>Interne</option>
                            <option value="Externe" {{ old('type', $reunion->type) == 'Externe' ? 'selected' : '' }}>Externe</option>
                            <option value="Autre" {{ old('type', $reunion->type) == 'Autre' ? 'selected' : '' }}>Autre</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="idDepartement" class="w-full bg-slate-50 border border-slate-200 focus:border-indigo-500 focus:bg-white rounded-xl px-4 py-3 text-sm outline-none transition-all">
                            ...
                            @foreach($departements as $dept)
                                <option value="{{ $dept->idDepartement }}" {{ old('idDepartement', $reunion->idDepartement) == $dept->idDepartement ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <input type="datetime-local" name="dateHeure" value="{{ old('dateHeure', \Carbon\Carbon::parse($reunion->dateHeure)->format('Y-m-d\TH:i')) }}" required class="w-full bg-slate-50 border border-slate-200 focus:border-indigo-500 focus:bg-white rounded-xl px-4 py-3 text-sm outline-none transition-all">
                    </div>

                    <div>
                        <label class="block text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1 mb-1.5">Heure de fin (Optionnel)</label>
                        <input type="time" name="heureFin" value="{{ old('heureFin', $reunion->heureFin) }}" class="w-full bg-slate-50 border border-slate-200 focus:border-indigo-500 focus:bg-white rounded-xl px-4 py-3 text-sm outline-none transition-all">
                    </div>

                    {{-- Location / Link --}}
                    <div>
                        <label class="block text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1 mb-1.5">Lieu / Salle</label>
                        <input type="text" name="lieu" value="{{ old('lieu', $reunion->lieu) }}" class="w-full bg-slate-50 border border-slate-200 focus:border-indigo-500 focus:bg-white rounded-xl px-4 py-3 text-sm outline-none transition-all">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="url" name="lien" value="{{ old('lien', $reunion->lien) }}" class="w-full bg-slate-50 border border-slate-200 focus:border-indigo-500 focus:bg-white rounded-xl px-4 py-3 text-sm outline-none transition-all">
                    </div>

                    {{-- Description --}}
                    <div class="md:col-span-2">
                        <label class="block text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1 mb-1.5">Objectif *</label>
                        <input type="text" name="objectif" value="{{ old('objectif', $reunion->objectif) }}" required class="w-full bg-slate-50 border border-slate-200 focus:border-indigo-500 focus:bg-white rounded-xl px-4 py-3 text-sm outline-none transition-all">
                    </div>

                    <div class="md:col-span-2">
                        <label class="block text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1 mb-1.5">Description détaillée</label>
                        <textarea name="description" rows="4" class="w-full bg-slate-50 border border-slate-200 focus:border-indigo-500 focus:bg-white rounded-xl px-4 py-3 text-sm outline-none transition-all">{{ old('description', $reunion->description) }}</textarea>
                    </div>
                </div>
