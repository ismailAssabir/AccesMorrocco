<x-app-layout>
    <div class="p-6 md:p-8 bg-[#F8FAFC] min-h-screen font-sans text-slate-900">
            ...
            <form id="filterForm" action="{{ route('admin.pointages.index') ?? url()->current() }}" method="GET"
                ...
                <div class="flex flex-wrap items-center justify-center gap-3 w-full mt-4 pt-4 border-t border-slate-100">
                    ...
                    <div id="customDatesContainer" class="flex items-center gap-2 {{ request('period') == 'custom' ? '' : 'hidden' }}">
                        <input type="date" name="start_date" value="{{ request('start_date') }}" onchange="fetchTableData()"
                        ...
                        <input type="date" name="end_date" value="{{ request('end_date') }}" onchange="fetchTableData()"
                            class="bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs font-bold text-slate-600 outline-none focus:border-[#be2346]">
                    </div>
                    ...
                    <div class="relative min-w-[150px]">
                        <select name="status" id="statusSelect" onchange="fetchTableData()"
                            ...
                            <option value="retard" {{ request('status') == 'retard' ? 'selected' : '' }}>En retard</option>
                        </select>
                        <div class="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
                            <svg class="h-3 w-3 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path d="M19 9l-7 7-7-7" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" />
                    ...
                    <div class="relative min-w-[200px]">
                        <select name="user_id" id="userSelect">
                            ...
                            @foreach($usersForFilter ?? [] as $u)
                                <option value="{{ $u->idUser }}" {{ request('user_id') == $u->idUser ? 'selected' : '' }}>
        ...
        <form id="clearHistoryForm" action="{{ route('admin.pointages.clear') }}" method="POST" class="hidden">
            @csrf
            @method('DELETE')
        </form>
    ...
    <script>
        ...
        function confirmClearHistory() {
            window.confirmDelete("{{ route('admin.pointages.clear') }}", 'historique');
        }
    </script>
