            ...
            <tr class="hover:bg-slate-50/80 transition-colors group">
                ...
                <td class="px-6 py-4">
                    <span class="text-[10px] font-black text-slate-400 uppercase">
                        {{ $user->typeContrat == 'CD' ? 'CDI' : ($user->typeContrat == 'CI' ? 'CI' : ($user->typeContrat ?: 'Non défini')) }}
                    </span>
                </td>
                <td class="px-6 py-4">
                    <span class="text-xs font-medium text-slate-500">{{ $user->departement->title ?? 'Aucun' }}</span>
                </td>
...
<div class="px-6 py-4 border-t border-slate-100 bg-white">
    {{ $users->links('vendor.pagination.tailwind_saas') }}
</div>
@endif
