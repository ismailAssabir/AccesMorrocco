<?php $__env->startSection('title', 'Journal de Voyage — Access Morocco'); ?>
<?php $__env->startSection('page-title', 'Journal de Voyage'); ?>
<?php $__env->startSection('page-subtitle', 'Gardez une trace de vos plus beaux moments au Maroc'); ?>
...
<div class="space-y-8 animate-fadeIn" x-data="{ 
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div class="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm">
            ...
            <p class="text-2xl font-black text-slate-800"><?php echo e($souvenirs->total()); ?></p>
        </div>
        <div class="bg-[#be2346]/5 p-6 rounded-[2rem] border border-[#be2346]/10 shadow-sm">
            ...
            <p class="text-xs font-bold text-slate-700 truncate"><?php echo e($souvenirs->first()->titre ?? 'Aucun'); ?></p>
        </div>
    </div>

    
    <?php if($souvenirs->isEmpty()): ?>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        ...
        <div class="group bg-white rounded-[2.5rem] border border-slate-100 shadow-sm hover:shadow-2xl hover:shadow-slate-200 transition-all duration-500 overflow-hidden flex flex-col h-full cursor-pointer"
             @click="openSouvenir(<?php echo e(json_encode($souvenir->toArray() + ['dossier_destination' => $souvenir->dossier->distination ?? null])); ?>)">
            
            <div class="relative aspect-[4/5] overflow-hidden bg-slate-100">
                <?php if($souvenir->image): ?>
                    <img src="<?php echo e(asset('storage/' . $souvenir->image)); ?>" alt="<?php echo e($souvenir->titre); ?>" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110">
                <?php else: ?>
                ...
                <div class="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-all translate-y-2 group-hover:translate-y-0" @click.stop>
                    <button type="button" 
                            @click="openEditSouvenir(<?php echo e(json_encode($souvenir->toArray() + ['dossier_destination' => $souvenir->dossier->distination ?? null])); ?>)"
                    ...
                    <button type="button" 
                            onclick="openGlobalDeleteModal('<?php echo e(route('clients.souvenirs.destroy', $souvenir->idSouvenir)); ?>', 'Supprimer ce souvenir ?', 'Êtes-vous sûr de vouloir retirer ce moment de votre journal ? Cette action est irréversible.')"
            ...
            <div class="p-6 flex flex-col flex-1">
                <div class="flex items-center gap-2 mb-3">
                    <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                        <?php echo e($souvenir->date ? \Carbon\Carbon::parse($souvenir->date)->format('d M Y') : $souvenir->created_at->format('d M Y')); ?>

                    </span>
                    <?php if($souvenir->location): ?>
                    <span class="w-1 h-1 rounded-full bg-slate-300"></span>
                    <span class="text-[10px] font-black text-[#be2346] uppercase tracking-widest truncate max-w-[100px]">
                        <?php echo e($souvenir->location); ?>

                    </span>
                    <?php endif; ?>
                </div>
                <h4 class="text-lg font-black text-slate-800 leading-tight mb-2 group-hover:text-[#be2346] transition-colors"><?php echo e($souvenir->titre); ?></h4>
                <p class="text-xs text-slate-500 leading-relaxed font-medium line-clamp-2 mb-6"><?php echo e($souvenir->description); ?></p>
                
                <?php if($souvenir->dossier): ?>
                <div class="mt-auto pt-4 border-t border-slate-50">
                    <div class="flex items-center gap-2">
                        ...
                        <span class="text-[9px] font-bold text-slate-400 uppercase tracking-tighter">Voyage à <?php echo e($souvenir->dossier->distination); ?></span>
                    </div>
                </div>
                <?php endif; ?>
            </div>
                            ...
                            <div>
                                ...
                                <select name="idDossier" x-model="selectedEditSouvenir.idDossier" class="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold text-slate-800 outline-none appearance-none">
                                    ...
                                    <?php $__currentLoopData = $dossiers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $dossier): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <option value="<?php echo e($dossier->idDossier); ?>">Voyage à <?php echo e($dossier->distination); ?></option>
                                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                </select>
                            </div>
                            <div>
    ...
    <div x-show="openAddModal" 
        ...
        <div class="relative w-full max-w-2xl bg-white rounded-[2.5rem] shadow-2xl shadow-slate-900/50 overflow-hidden flex flex-col"
            ...
            <div class="flex-1 max-h-[75vh] overflow-y-auto custom-scrollbar bg-white">
                <form action="<?php echo e(route('clients.souvenirs.store')); ?>" method="POST" enctype="multipart/form-data" class="p-8 space-y-8">
                    <?php echo csrf_field(); ?>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <select name="idDossier" class="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold text-slate-800 focus:border-[#be2346] focus:ring-4 focus:ring-[#be2346]/5 transition-all outline-none appearance-none">
                                    <option value="">Voyage libre</option>
                                    <?php $__currentLoopData = $dossiers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $dossier): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <option value="<?php echo e($dossier->idDossier); ?>">
                                            Voyage à <?php echo e($dossier->distination); ?> <?php echo e($dossier->dateVoyage ? '(' . \Carbon\Carbon::parse($dossier->dateVoyage)->format('F Y') . ')' : ''); ?>
...
<div x-data="{ show: true }" 
    ...
    <p class="text-sm font-bold"><?php echo e(session('msg')); ?></p>
...
<?php echo $__env->make('layouts.client', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\dell\Desktop\AccesMorrocco\resources\views/clients/souvenirs/index.blade.php ENDPATH**/ ?>