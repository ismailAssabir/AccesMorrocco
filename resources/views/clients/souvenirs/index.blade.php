...
<div x-data="{ 
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <button @click="openAddModal = true" class="flex items-center justify-center gap-2 px-6 py-3.5 bg-[#b11d40] text-white rounded-2xl font-black text-sm shadow-xl shadow-[#b11d40]/20 hover:scale-[1.02] active:scale-95 transition-all group">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div class="bg-[#b11d40]/5 p-6 rounded-[2rem] border border-[#b11d40]/10 shadow-sm">
            <p class="text-[10px] font-black text-[#b11d40] uppercase tracking-widest mb-1">Dernier Moment</p>
    ...
    <div class="bg-white rounded-[3rem] border border-dashed border-slate-200 p-16 text-center">
        ...
        <button @click="openAddModal = true" class="text-[#b11d40] font-black text-sm uppercase tracking-widest hover:underline">Créer mon premier souvenir</button>
    </div>
    @else
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        ...
        <div class="group bg-white rounded-[2.5rem] border border-slate-100 shadow-sm hover:shadow-2xl hover:shadow-slate-200 transition-all duration-500 overflow-hidden flex flex-col h-full cursor-pointer"
            ...
            <div class="relative aspect-[4/5] overflow-hidden bg-slate-100">
                ...
                {{-- Mood Tag --}}
                @if($souvenir->mood)
                <div class="absolute top-4 left-4 px-3 py-1.5 bg-white/90 backdrop-blur-md rounded-full text-[9px] shadow-sm font-black uppercase tracking-widest text-[#b11d40] border border-[#b11d40]/10">
                    {{ $souvenir->mood }}
                </div>
                @endif
                ...
                <div class="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-all translate-y-2 group-hover:translate-y-0" @click.stop>
                    <button type="button" 
                            ...
                            class="w-8 h-8 bg-white/90 backdrop-blur-md text-slate-600 rounded-xl flex items-center justify-center hover:bg-[#b11d40] hover:text-white transition-all shadow-lg">
            ...
            <div class="p-6 flex flex-col flex-1">
                <div class="flex items-center gap-2 mb-3">
                    ...
                    <span class="text-[10px] font-black text-[#b11d40] uppercase tracking-widest truncate max-w-[100px]">
                        {{ $souvenir->location }}
                    </span>
                    @endif
                </div>
                <h4 class="text-lg font-black text-slate-800 leading-tight mb-2 group-hover:text-[#b11d40] transition-colors">{{ $souvenir->titre }}</h4>
        ...
        <div class="relative w-full max-w-5xl bg-white rounded-3xl shadow-xl overflow-hidden flex flex-col md:flex-row h-full max-h-[85vh] border-none">
            ...
            <div class="flex-1 bg-slate-50 relative flex items-center justify-center group overflow-hidden">
                <template x-if="selectedSouvenir && selectedSouvenir.image">
                    <img :src="'/storage/' + selectedSouvenir.image" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105">
                </template>
                <template x-if="selectedSouvenir && !selectedSouvenir.image">
                ...
                <template x-if="selectedSouvenir && selectedSouvenir.image">
                    <a :href="'/storage/' + selectedSouvenir.image" download 
                       class="absolute bottom-8 right-8 flex items-center gap-3 px-6 py-3 bg-white/20 backdrop-blur-md text-white rounded-2xl font-black text-xs hover:bg-white hover:text-[#b11d40] transition-all shadow-xl">
            ...
            <div class="w-full md:w-[400px] bg-white flex flex-col p-10 overflow-y-auto custom-scrollbar">
                <div class="flex items-center justify-between mb-8">
                    <div class="flex gap-2">
                        <div class="px-4 py-1.5 bg-[#b11d40]/5 rounded-full border border-[#b11d40]/10">
                            <span class="text-[10px] font-black text-[#b11d40] uppercase tracking-widest" x-text="selectedSouvenir?.mood || 'Souvenir'"></span>
                        </div>
                        <button @click="openEditSouvenir(selectedSouvenir)" class="w-8 h-8 rounded-xl bg-slate-50 flex items-center justify-center text-slate-400 hover:text-[#b11d40] transition-all border border-slate-100">
                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" stroke-width="2"/></svg>
                        </button>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-4 text-slate-400">
                            ...
                            <template x-if="selectedSouvenir?.location">
                                <div class="flex items-center gap-1.5">
                                    <svg class="w-3.5 h-3.5 text-[#b11d40]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" stroke-width="2"/></svg>
                                    <span class="text-[10px] font-bold uppercase text-[#b11d40]" x-text="selectedSouvenir?.location"></span>
                                </div>
                            </template>
                        </div>
                    </div>

                    <div class="w-12 h-1 bg-[#b11d40]/20 rounded-full"></div>
                        ...
                        <div class="pt-8 mt-8 border-t border-slate-50">
                            ...
                            <div class="p-4 bg-slate-50 rounded-2xl border border-slate-100 flex items-center gap-3">
                                <div class="w-10 h-10 rounded-xl bg-white shadow-sm flex items-center justify-center">
                                    <svg class="w-5 h-5 text-[#b11d40]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z" stroke-width="2"/></svg>
                                </div>
                                <div>
            ...
            <div class="flex-1 max-h-[75vh] overflow-y-auto custom-scrollbar bg-white">
                <form :action="'/clients/journal/' + selectedEditSouvenir?.idSouvenir" method="POST" enctype="multipart/form-data" class="px-8 pb-8 pt-0 space-y-8">
                    ...
                    <div class="space-y-6">
                        <div class="flex items-center gap-3">
                            <div class="w-8 h-8 rounded-xl bg-slate-50 border border-slate-100 flex items-center justify-center">
                                <svg class="w-4 h-4 text-[#b11d40]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" stroke-width="2"/></svg>
                            </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="md:col-span-2">
                                ...
                                <input type="text" name="titre" required x-model="selectedEditSouvenir.titre"
                                       class="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold text-slate-800 focus:border-[#b11d40] transition-all outline-none">
                            </div>
                            <div>
                    ...
                    <div class="space-y-6">
                        <div class="flex items-center gap-3">
                            <div class="w-8 h-8 rounded-xl bg-slate-50 border border-slate-100 flex items-center justify-center">
                                <svg class="w-4 h-4 text-[#b11d40]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M14.828 14.828a4 4 0 01-5.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" stroke-width="2"/></svg>
                            </div>
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                            <template x-for="m in [
                                ...
                                <button type="button" @click="selectedEditSouvenir.mood = m.id" 
                                        :class="selectedEditSouvenir.mood === m.id ? 'border-[#b11d40] bg-[#b11d40]/5 text-[#b11d40]' : 'border-slate-100 bg-slate-50/50 text-slate-400'"
                    ...
                    <div class="space-y-6">
                        <div class="flex items-center gap-3">
                            <div class="w-8 h-8 rounded-xl bg-slate-50 border border-slate-100 flex items-center justify-center">
                                <svg class="w-4 h-4 text-[#b11d40]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" stroke-width="2"/></svg>
                            </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="location" x-model="selectedEditSouvenir.location" 
                                   class="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold text-slate-800 focus:border-[#b11d40] outline-none">
                        </div>
                        <div>
                            ...
                            <textarea name="description" rows="4" x-model="selectedEditSouvenir.description"
                                      class="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-medium text-slate-700 focus:border-[#b11d40] outline-none resize-none"></textarea>
                        </div>
                        <div x-data="{ hasFile: false, fileName: '' }">
                            ...
                            <div class="relative">
                                ...
                                <label for="souvenir_image_edit" 
                                       :class="hasFile ? 'border-[#b11d40] bg-[#b11d40]/5' : 'border-slate-200 bg-slate-50 hover:border-[#b11d40]/30'"
                                    ...
                                    <div class="w-12 h-12 rounded-2xl bg-white shadow-sm flex items-center justify-center mb-3 group-hover:scale-110 transition-transform">
                                        <svg class="w-6 h-6 text-[#b11d40]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" stroke-width="2" /></svg>
                                    </div>
                                    <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest group-hover:text-[#b11d40]" x-text="hasFile ? fileName : 'Cliquez pour remplacer la photo'"></p>
                    ...
                    <div class="pt-6 border-t border-slate-50 flex gap-4">
                        <button type="submit" class="flex-1 py-5 bg-[#b11d40] text-white rounded-3xl font-black text-sm shadow-xl shadow-[#b11d40]/20 hover:scale-[1.02] transition-all">
                            Mettre à jour
                        </button>
            ...
            <div class="flex-1 max-h-[75vh] overflow-y-auto custom-scrollbar bg-white">
                <form action="{{ route('clients.souvenirs.store') }}" method="POST" enctype="multipart/form-data" class="px-8 pb-8 pt-0 space-y-8">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="md:col-span-2">
                                ...
                                <input type="text" name="titre" required placeholder="Ex: Une nuit magique sous les étoiles" 
                                       class="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold text-slate-800 focus:border-[#b11d40] focus:ring-4 focus:ring-[#b11d40]/5 transition-all outline-none">
                            </div>
                            <div>
                                <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block ml-1">Lié au Voyage</label>
                                <select name="idDossier" class="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold text-slate-800 focus:border-[#b11d40] focus:ring-4 focus:ring-[#b11d40]/5 transition-all outline-none appearance-none">
                                    <option value="">Voyage libre</option>
                                    @foreach($dossiers as $dossier)
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                            <template x-for="m in [
                                ...
                                <button type="button" @click="currentMood = m.id" 
                                        :class="currentMood === m.id ? 'border-[#b11d40] ring-4 ring-[#b11d40]/5 bg-[#b11d40]/5 text-[#b11d40]' : 'border-slate-100 bg-slate-50/50 hover:border-[#b11d40]/30 text-slate-400'"
                        ...
                        <div class="grid grid-cols-1 gap-6">
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="text" name="location" placeholder="Ex: Jardin Majorelle, Marrakech" 
                                           class="w-full pl-12 pr-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold text-slate-800 focus:border-[#b11d40] focus:ring-4 focus:ring-[#b11d40]/5 transition-all outline-none">
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <textarea name="description" rows="4" placeholder="Décrivez ce moment spécial..." 
                                          class="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-medium text-slate-700 focus:border-[#b11d40] focus:ring-4 focus:ring-[#b11d40]/5 transition-all outline-none resize-none"></textarea>
                            </div>
                            ...
                            <div x-data="{ hasFile: false, fileName: '' }">
                                ...
                                <div class="relative">
                                    ...
                                    <label for="souvenir_image_new" 
                                        ...
                                        <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest group-hover:text-[#b11d40]" x-text="hasFile ? fileName : 'Sélectionner une photo'"></p>
                                    </label>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="pt-6 border-t border-slate-50 flex flex-col sm:flex-row items-center gap-4">
                        <button type="submit" class="w-full sm:flex-1 py-5 bg-[#b11d40] text-white rounded-3xl font-black text-sm shadow-xl shadow-[#b11d40]/20 hover:scale-[1.02] active:scale-95 transition-all">
...
<div x-data="{ show: true }" 
    ...
    <div class="w-8 h-8 bg-[#b11d40] rounded-xl flex items-center justify-center">
