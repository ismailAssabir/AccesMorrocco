            ...
            <div class="px-6 py-5 border-b border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-50/50">
                ...
                <div class="flex flex-wrap items-center gap-3">
                    ...
                    <div class="bg-white p-1 rounded-xl border border-slate-200 shadow-sm flex items-center">
                        <a href="{{ route('departements.show', ['departement' => $departement->idDepartement, 'period' => 'today']) }}" 
                        ...
                        <a href="{{ route('departements.show', ['departement' => $departement->idDepartement, 'period' => 'weekly']) }}" 
                        ...
                        <a href="{{ route('departements.show', ['departement' => $departement->idDepartement, 'period' => 'monthly']) }}" 
                                     ...
                                     <div class="flex flex-col items-center gap-1.5">
                                         <div class="flex items-center gap-3">
                                             <div class="flex items-center gap-2">
                                                 <span class="text-xs font-extrabold {{ ($employee->presence_percentage ?? 0) >= 80 ? 'text-emerald-500' : (($employee->presence_percentage ?? 0) >= 40 ? 'text-amber-500' : 'text-red-500') }}">
                                                     {{ $employee->presence_percentage ?? 0 }}%
                                                 </span>
                                                 <div class="w-16 bg-slate-100 rounded-full h-1.5 overflow-hidden">
                                                     <div class="h-1.5 rounded-full transition-all duration-500 {{ ($employee->presence_percentage ?? 0) >= 80 ? 'bg-emerald-400' : (($employee->presence_percentage ?? 0) >= 40 ? 'bg-amber-400' : 'bg-red-400') }}" 
                                             ...
                                             @if($employee->is_here_today)
                                                 ...
                                                 @endif
                                             @else
                                                 @if($employee->today_pointage && strtolower($employee->today_pointage->status) === 'absent')
