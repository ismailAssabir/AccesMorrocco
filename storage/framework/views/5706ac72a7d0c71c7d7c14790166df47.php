    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
            <h1 class="text-2xl font-extrabold tracking-tight text-slate-800">
                <?php echo e($isAdmin ? 'Centre de Contrôle des Présences' : 'Mon Pointage'); ?>

            </h1>
            <p class="text-slate-500 text-sm mt-1 font-medium">
                <?php echo e($isAdmin ? 'Suivi global des présences et gestion des pointages de l\'équipe.' : 'Enregistrez votre arrivée et votre départ via GPS.'); ?>
...
<?php endif; ?>
    ...
    <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('can_point')): ?>
    ...
    <?php if($isAdmin): ?>
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            <div class="bg-white border border-slate-200 p-5 rounded-2xl shadow-sm flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-2xl font-extrabold text-slate-800"><?php echo e($totalToday); ?></p>
                </div>
            </div>
            <div class="bg-white border border-slate-200 p-5 rounded-2xl shadow-sm flex items-center gap-4 border-l-4 border-l-emerald-400">
                ...
                <div>
                    ...
                    <p class="text-2xl font-extrabold text-emerald-500"><?php echo e($presentsToday); ?></p>
                </div>
            </div>
            <div class="bg-white border border-slate-200 p-5 rounded-2xl shadow-sm flex items-center gap-4 border-l-4 border-l-amber-400">
                ...
                <div>
                    ...
                    <p class="text-2xl font-extrabold text-amber-500"><?php echo e($retardsToday); ?></p>
                </div>
            </div>
            <div class="bg-white border border-slate-200 p-5 rounded-2xl shadow-sm flex items-center gap-4 border-l-4 border-l-[#b11d40]">
                ...
                <div>
                    ...
                    <p class="text-2xl font-extrabold text-[#b11d40]"><?php echo e($absentsToday); ?></p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white border border-slate-200 rounded-2xl shadow-sm p-4 mb-6">
            <form id="filterForm" action="<?php echo e(route('pointages.index')); ?>" method="GET" class="flex flex-wrap items-center gap-4">
                ...
                <div class="flex flex-wrap items-center gap-3">
                    ...
                    <a href="<?php echo e(route('pointages.index')); ?>" class="p-3 rounded-xl bg-slate-100 text-slate-500 hover:bg-slate-200 hover:text-slate-800 transition-all flex items-center justify-center" title="Réinitialiser les filtres">
    ...
    <div class="bg-white border border-slate-200 rounded-3xl shadow-sm overflow-hidden mb-8">
        ...
        <div class="px-7 pt-6 pb-2 flex items-center justify-between border-b border-slate-50 bg-slate-50/30">
            <div>
                <h2 class="text-lg font-black text-slate-800">
                    <?php echo e($isAdmin ? 'Suivi Global (Aujourd\'hui)' : 'Historique de Pointage'); ?>

                </h2>
                <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-0.5">
                    <?php echo e($isAdmin ? 'Tous les enregistrements de la journée' : 'Vos 15 derniers enregistrements'); ?>
        ...
        <?php if($recentPointages->isNotEmpty()): ?>
            <div class="overflow-x-auto">
                <table class="w-full text-left text-sm text-slate-600">
                    <thead class="bg-slate-50 border-b border-slate-200 text-xs uppercase font-extrabold text-slate-400 tracking-wider">
                        <tr>
                            <th class="px-6 py-4">Date & <?php echo e($isAdmin ? 'Employé' : 'Rôle'); ?></th>
                        ...
                        <?php $__currentLoopData = $recentPointages; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $p): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <tr class="hover:bg-slate-50/80 transition-colors group">
                                <td class="px-6 py-4 font-bold text-slate-800 flex flex-col">
                                    <span class="text-[10px] text-slate-400 mb-1"><?php echo e(\Carbon\Carbon::parse($p->date)->translatedFormat('d M Y')); ?></span>
                                    <?php if($isAdmin): ?>
                                        <span class="text-[12px] font-black text-slate-800 group-hover:text-[#be2346] transition-colors leading-tight">
                                            <?php echo e($p->user->firstName ?? ''); ?> <?php echo e($p->user->lastName ?? ''); ?>

                                        </span>
                                        <span class="text-[9px] font-bold text-slate-400 uppercase tracking-tighter"><?php echo e($p->user->post ?? ($p->user->departement->title ?? 'Poste non défini')); ?></span>
                                    <?php else: ?>
                                        <span class="text-[11px] font-black uppercase text-slate-600 tracking-widest"><?php echo e(auth()->user()->type); ?></span>
                                    <?php endif; ?>
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4 text-center">
                                    <?php if($isAdmin): ?>
                                        <?php if($p->justification): ?>
                                            <div class="flex items-center justify-center gap-1.5">
                                                <?php if($p->justification_status === 'en_attente'): ?>
                                                    <button type="button" 
                                                        onclick="openAdminJustifModal({
                                                            id: <?php echo e($p->idPointage); ?>,
                                                            user: '<?php echo e($p->user->firstName); ?> <?php echo e($p->user->lastName); ?>',
                                                            type: '<?php echo e($p->typejustif); ?>',
                                                            motif: '<?php echo e(addslashes($p->justification)); ?>',
                                                            date: '<?php echo e(\Carbon\Carbon::parse($p->date)->translatedFormat('d M Y')); ?>',
                                                            fichier: '<?php echo e($p->fichier); ?>'
                                                        })"
                                                        class="bg-[#be2346] hover:bg-[#a01d3a] text-white text-[9px] font-black uppercase px-3 py-1.5 rounded-lg transition-all shadow-sm">
                                                        Justification 📥
                                                    </button>
                                                <?php else: ?>
                                                    <span class="text-[9px] font-black uppercase <?php echo e($p->justification_status === 'accepte' ? 'text-emerald-500' : 'text-[#be2346]'); ?> bg-slate-50 px-2 py-1 rounded-lg border border-slate-100">
                                                        <?php echo e($p->justification_status === 'accepte' ? 'Validé ✓' : 'Refusé ✗'); ?>

                                                    </span>
                                                <?php endif; ?>
                                            </div>
                                        <?php elseif($p->status !== 'present'): ?>
                                    ...
                                    <?php else: ?>
                                        <?php if($p->justification): ?>
                                            <div class="flex items-center justify-center gap-1.5">
                                                <span class="text-[9px] font-black uppercase <?php echo e($p->justification_status === 'accepte' ? 'text-emerald-500' : ($p->justification_status === 'refuse' ? 'text-[#be2346]' : 'text-amber-500')); ?> bg-slate-50 px-2 py-1 rounded-lg border border-slate-100">
                                                    <?php echo e($p->justification_status === 'en_attente' ? 'En attente' : ($p->justification_status === 'accepte' ? 'Validé' : 'Refusé')); ?>

                                                </span>
                                                <?php if($p->justification_status === 'refuse'): ?>
                                                    <button type="button" onclick="openRefusalReasonModal('<?php echo e(addslashes($p->motif_refus)); ?>')" class="p-1.5 rounded-lg bg-red-50 text-[#be2346] hover:bg-red-100 transition-all" title="Voir le motif du refus">
                                        ...
                                        <?php elseif($p->status !== 'present'): ?>
                                            <button onclick="openJustifModal(<?php echo e($p->idPointage); ?>)" class="text-[10px] font-black text-[#be2346] hover:bg-[#be2346] hover:text-white border border-[#be2346] px-3 py-1 rounded-lg transition-all uppercase tracking-widest">Justifier</button>
                                        <?php else: ?>
                                            <span class="text-slate-300 font-black text-[10px]">--</span>
                                        <?php endif; ?>
                                    <?php endif; ?>
            ...
            <?php if(method_exists($recentPointages, 'links') && $recentPointages->hasPages()): ?>
                <div class="px-6 py-4 border-t border-slate-100 bg-white">
                    <?php echo e($recentPointages->links('vendor.pagination.tailwind_saas')); ?>
...
<div id="justifModal" class="fixed inset-0 z-[110] hidden items-center justify-center p-4">
    ...
    <div class="relative bg-white w-full max-w-lg rounded-[32px] shadow-2xl overflow-hidden flex flex-col z-10" style="animation: modalIn .2s ease-out">
        ...
        <form action="<?php echo e(route('justification.submit')); ?>" method="POST" enctype="multipart/form-data" class="p-7 space-y-5">
            <?php echo csrf_field(); ?> <input type="hidden" name="idPointage" id="justif-idPointage">
            ...
            <div class="p-7 space-y-6">
                ...
                <div class="flex gap-3 pt-2">
                    <form id="aj-accept-form" method="POST" class="flex-1">
                        <?php echo csrf_field(); ?> <input type="hidden" name="action" value="accepte">
    ...
    <div id="adminRefuseModal" class="fixed inset-0 z-[120] hidden items-center justify-center p-4">
        ...
        <div class="relative bg-white w-full max-w-md rounded-[32px] shadow-2xl overflow-hidden flex flex-col z-10" style="animation: modalIn .2s ease-out">
            ...
            <form id="adminRefuseForm" method="POST" class="p-7 space-y-5">
                <?php echo csrf_field(); ?> <input type="hidden" name="action" value="refuse">
                <div class="space-y-1.5">
...
<script>
    ...
    async function getLocationAndSubmit(action) {
        ...
        navigator.geolocation.getCurrentPosition(async (pos) => {
            ...
            try {
                const res = await fetch(action === 'checkin' ? '<?php echo e(route("pointage.checkin")); ?>' : '<?php echo e(route("pointage.checkout")); ?>', {
...
<?php endif; ?>
<?php /**PATH C:\Users\4B\Desktop\ExercicesLaravel\voyage\resources\views/pointages/index.blade.php ENDPATH**/ ?>