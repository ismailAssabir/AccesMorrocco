                ...
                <div class="relative">
                    <select id="dept-filter" onchange="filterTable()" class="appearance-none bg-slate-50 border border-slate-200 rounded-xl pl-4 pr-10 py-2.5 text-xs font-bold text-slate-600 outline-none focus:border-[#be2346] transition-all cursor-pointer">
                        ...
                        @foreach(\App\Models\Departement::orderBy('title')->get() as $dept)
                            <option value="{{ strtolower($dept->title) }}">{{ $dept->title }}</option>
                        @endforeach
                    </select>
                    <div class="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
                        <svg class="h-3 w-3 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M19 9l-7 7-7-7" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"/></svg>
                                ...
                                <div class="space-y-2">
                                    ...
                                    @if($pointage->justification_status === 'en_attente')
                                        <div class="flex items-center gap-2 pt-1">
                                            ...
                                            <button type="button" onclick="openAdminRefuseModal({{ $pointage->idPointage }})" class="text-[10px] bg-slate-200 hover:bg-slate-300 text-slate-700 font-bold px-2 py-1 rounded transition-colors">Refuser</button>
                                        </div>
                                    @endif
                                    @if($pointage->justification_status === 'refuse' && $pointage->motif_refus)
                                        <div class="mt-1 p-1.5 bg-red-50 border border-red-100 rounded text-[9px] text-red-700">
                                            <strong class="font-bold">Motif refus:</strong> {{ $pointage->motif_refus }}
                                        </div>
                                    @endif
                                </div>
                            @else
        ...
        <form action="{{ route('admin.settings.update') }}" method="POST" class="p-7 space-y-5">
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="number" name="maxDelay" placeholder="Ex: 15" min="0" max="120" value="{{ $settings->maxDelay ?? '' }}"
                        class="w-full bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3 text-sm outline-none transition-all focus:border-[#be2346] focus:ring-4 focus:ring-[#be2346]/5">
                </div>
            </div>
