    ...
    <div class="p-8 bg-[#F8FAFC] min-h-screen">
        ...
        <div class="mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <div class="flex gap-3">
                ...
                @can('dossier.delete')
                <button type="button"
                        onclick="window.confirmDelete('{{ route('dossiers.destroy', $dossier->idDossier) }}', 'dossier')"
                    ...
                    <div class="p-6">
                        ...
                        <div class="p-4 bg-slate-50 rounded-2xl mb-3 border border-slate-100 group hover:border-[#b11d40]/20 transition-all">
                                    ...
                                    <div class="flex items-center gap-2">
                                        ...
                                        <div class="flex items-center" x-data="{ 
                                            ...
                                            async updateStatus(newStatus) {
                                                try {
                                                    const res = await fetch(`/admin/presentations/{{ $presentation->idPresentation }}`, {
                                                        method: 'PUT',
                                                        headers: {
                                        ...
                                        <button @click="openModal({{ json_encode($presentation) }})" class="p-1 text-slate-400 hover:text-[#b11d40] transition-colors opacity-0 group-hover:opacity-100" title="Modifier">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                            </svg>
                                        </button>
                                        <button @click="duplicatePresentation({{ $presentation->idPresentation }})" class="p-1 text-slate-400 hover:text-green-600 transition-colors opacity-0 group-hover:opacity-100" title="Nouvelle Version">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7v8a2 2 0 002 2h6M8 7V5a2 2 0 012-2h4.586a1 1 0 01.707.293l4.414 4.414a1 1 0 01.293.707V15a2 2 0 01-2 2h-2M8 7H6a2 2 0 00-2 2v10a2 2 0 002 2h8a2 2 0 002-2v-2" />
                                            </svg>
                                        </button>
                                        <button @click="deletePresentation({{ $presentation->idPresentation }})" class="p-1 text-slate-400 hover:text-red-600 transition-colors opacity-0 group-hover:opacity-100" title="Supprimer">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                                            </svg>
                                        </button>
                            ...
                            <div class="mt-3 space-y-1 pl-2 border-l-2 border-slate-200 group-hover:border-[#b11d40]/30 transition-all">
                                ...
                                <div class="flex justify-between text-[11px] text-slate-500 p-1.5 rounded-lg transition-all {{ $item->status === 'validee' ? 'bg-green-50/50' : '' }}">
                                    <span class="flex items-center gap-2">
                                        @if($item->status === 'validee')
                                        ...
                                        <span class="flex flex-col">
                                            <span class="font-black text-slate-800 text-xs">{{ $item->nom }}</span>
                                            <span class="text-[9px] text-slate-400 font-bold uppercase tracking-tighter">{{ $item->category->nom ?? 'Article' }}</span>
                                        </span>
                                    </span>
                                    <div class="flex items-center gap-3">
                                        <span class="text-[9px] font-bold text-slate-500">x{{ $item->quantity }}</span>
                                        <span class="font-bold text-slate-700">{{ number_format($item->totale, 2) }} MAD</span>
                                    </div>
                                </div>
                                @endforeach
                            </div>
                            ...
                            <div class="mt-4 p-3 bg-[#b11d40]/5 rounded-xl border border-[#b11d40]/10">
                                ...
                                <p class="text-[10px] text-slate-600 italic leading-relaxed">"{{ Str::limit($presentation->reponse, 100) }}"</p>
                            </div>
                            @endif
                        </div>
                        @empty
                        ...
                        <div x-show="showModal"
                            ...
                            <div class="px-8 py-6 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between shrink-0">
                                <div>
                                    ...
                                    <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-0.5">Dossier: {{ $dossier->reference }}</p>
                                </div>
