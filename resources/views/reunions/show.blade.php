            ...
            <nav class="flex mb-8 text-sm font-medium" aria-label="Breadcrumb">
                <ol class="inline-flex items-center space-x-1 md:space-x-3">
                    <li class="inline-flex items-center">
                        <a href="{{ route('reunions.index') }}" class="text-gray-500 hover:text-[#be2346] transition-colors flex items-center">
            ...
            <div class="bg-white rounded-[2.5rem] border border-gray-100 shadow-2xl shadow-gray-200/50 overflow-hidden">
                ...
                <div class="bg-gradient-to-br from-[#be2346] to-rose-700 p-8 md:p-12 text-white relative overflow-hidden">
                    ...
                    <div class="relative z-10">
                        ...
                        <h1 class="text-3xl md:text-5xl font-black mb-4 leading-tight">{{ $reunion->titre }}</h1>
                        <p class="text-rose-100 text-lg font-medium opacity-90">{{ $reunion->objectif }}</p>
                    </div>
                </div>
                ...
                <div class="p-8 md:p-12">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-12">
                        ...
                        <div class="space-y-8">
                                ...
                                <div class="space-y-6">
                                    <div class="flex items-start gap-4">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-lg font-black text-gray-900">{{ $reunion->dateHeure->translatedFormat('l d F Y') }}</p>
                                            <p class="text-sm font-bold text-gray-500">À partir de {{ $reunion->dateHeure->format('H:i') }}</p>
                                        </div>
                                    </div>
                                    ...
                                    <div class="flex items-start gap-4">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-lg font-black text-gray-900">{{ \Carbon\Carbon::parse($reunion->heureFin)->format('H:i') }}</p>
                                        </div>
                                    </div>
                                    @endif
                                    ...
                                    <div class="flex items-start gap-4">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-lg font-black text-gray-900">{{ $reunion->lieu ?? 'Visioconférence' }}</p>
                                        </div>
                                    </div>
                                        ...
                                        <div>
                                            ...
                                            <p class="text-lg font-black text-gray-900">
                                                @if($reunion->idDepartement)
                                                    Département: {{ $reunion->departement->title ?? 'N/A' }}
                                                @elseif($reunion->participants->count() > 0)
                                                    {{ $reunion->participants->count() }} Participant(s) spécifique(s)
                                                @else
                                                    Tous les employés
                                                @endif
                                            </p>
                            ...
                            @if($reunion->participants->count() > 0)
                            <div>
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    ...
                                    <div class="flex items-center gap-3 p-3 bg-white border border-gray-100 rounded-2xl shadow-sm">
                                        <div class="w-8 h-8 rounded-full bg-[#be2346]/10 flex items-center justify-center text-[#be2346] font-black text-[10px]">
                                            {{ substr($participant->firstName, 0, 1) }}{{ substr($participant->lastName, 0, 1) }}
                                        </div>
                                        <div class="min-w-0">
                                            <p class="text-xs font-bold text-gray-900 truncate">{{ $participant->firstName }} {{ $participant->lastName }}</p>
                                            <p class="text-[9px] text-gray-400 font-medium truncate">{{ $participant->post }}</p>
                                        </div>
                                    </div>
                                    @endforeach
                                </div>
                        ...
                        <div class="space-y-8">
                            <div>
                                ...
                                <div class="bg-gray-50 p-6 rounded-3xl border border-gray-100">
                                    <p class="text-gray-600 leading-relaxed italic">
                                        {{ $reunion->description ?? 'Aucune description supplémentaire fournie pour cette réunion.' }}
                                    </p>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <a href="{{ $reunion->lien }}" target="_blank" class="group flex items-center justify-between p-6 bg-gray-900 rounded-[2rem] text-white hover:bg-[#be2346] transition-all duration-300 shadow-xl shadow-gray-900/10">
                            ...
                            <div class="flex gap-4 pt-8">
                                <a href="{{ route('reunions.index') }}" class="flex-1 inline-flex items-center justify-center px-6 py-4 bg-white border border-gray-200 rounded-2xl text-sm font-black text-gray-500 hover:bg-gray-50 hover:text-gray-900 transition-all shadow-sm">
                                    Retour à la liste
                                </a>
                            </div>
                        </div>
