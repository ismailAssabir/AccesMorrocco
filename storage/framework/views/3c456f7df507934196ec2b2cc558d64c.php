    ...
    <table class="stats-table">
        <tr>
            ...
            <td class="stat-card bg-leads" width="25%">
                ...
                <div class="stat-card-value"><?php echo e($leads->whereIn('statut', ['nouveau', '1er_appel', '2eme_appel', 'promis'])->count()); ?></div>
            </td>
            <td class="stat-card bg-converted" width="25%">
                <div class="stat-card-title">Convertis</div>
                <div class="stat-card-value"><?php echo e($leads->where('statut', 'ok')->count()); ?></div>
            </td>
            <td class="stat-card bg-lost" width="25%">
                <div class="stat-card-title">Perdus</div>
                <div class="stat-card-value"><?php echo e($leads->where('statut', 'lost')->count()); ?></div>
            </td>
        </tr>
    </table>
    ...
    <table class="data-table">
        ...
        <tbody>
            <?php $__empty_1 = true; $__currentLoopData = $leads; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $lead): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <tr>
                    <td>
                        <div class="lead-name"><?php echo e(!empty($lead->firstName) ? $lead->firstName : 'Inconnu'); ?> <?php echo e(!empty($lead->lastName) ? $lead->lastName : ''); ?></div>
                    </td>
                    <td>
                        <div class="contact-email"><?php echo e(!empty(trim($lead->email ?? '')) ? $lead->email : 'Email non renseigné'); ?></div>
                        <div class="contact-phone"><?php echo e(!empty(trim($lead->phoneNumber ?? '')) ? $lead->phoneNumber : 'Tél non renseigné'); ?></div>
                    </td>
                    <td style="text-align: center;">
                        <span class="badge badge-type"><?php echo e(!empty(trim($lead->type ?? '')) ? $lead->type : 'Non défini'); ?></span>
                    </td>
                    <td><?php echo e(!empty(trim($lead->source ?? '')) ? $lead->source : 'Non définie'); ?></td>
                    <td>
                        <div style="font-weight: bold; color: #334155;"><?php echo e(!empty(trim($lead->nationalite ?? '')) ? $lead->nationalite : 'Non définie'); ?></div>
                        <div class="lead-address" style="margin-top: 3px;"><?php echo e(!empty($lead->address) ? $lead->address : 'Adresse non renseignée'); ?></div>
                    </td>
                    <td><?php echo e(!empty(trim($lead->departements->title ?? '')) ? $lead->departements->title : 'Non spécifié'); ?></td>
                    <td><?php echo e(!empty($lead->user) ? $lead->user->firstName . ' ' . $lead->user->lastName : 'Non assigné'); ?></td>
                    <td style="text-align: center; font-weight: bold; color: #475569;">
                        <?php echo e(!empty($lead->dateCreation) ? \Carbon\Carbon::parse($lead->dateCreation)->format('d/m/Y') : 'Inconnue'); ?>

                    </td>
                    <td style="text-align: center;">
                        <div class="badge badge-<?php echo e(strtolower(!empty($lead->statut) ? $lead->statut : 'nouveau')); ?>">
                            <?php
                                $statusLabels = [
                                    'nouveau' => 'Nouveau',
                                    '1er_appel' => '1er Appel',
                            ...
                            ?>
                            <?php echo e($statusLabels[$statut] ?? $statut); ?>
