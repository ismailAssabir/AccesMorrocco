        ...
        <div class="bg-white border border-slate-200 rounded-3xl shadow-sm overflow-hidden max-w-4xl mx-auto">
            ...
            <div class="p-8">
                <div class="flex items-center gap-4 mb-8">
                    ...
                    <div>
                        <h2 class="text-2xl font-extrabold text-slate-800">{{ $category->nom }}</h2>
                        <p class="text-slate-500 text-sm">ID: #{{ $category->id }}</p>
                    </div>
                </div>
                ...
                <div class="space-y-6">
                    <div class="border-b border-slate-100 pb-4">
                        ...
                        <p class="text-slate-800 text-lg font-semibold">{{ $category->nom }}</p>
                    </div>

                    <div class="border-b border-slate-100 pb-4">
                        <label class="block text-xs font-black text-slate-500 uppercase mb-2">Description</label>
                        <div class="text-slate-700 leading-relaxed">
                            {{ $category->desc ?: 'Aucune description disponible' }}
                        </div>
                    </div>
                    ...
                    <div class="border-b border-slate-100 pb-4">
                        ...
                        <div class="grid grid-cols-2 gap-4 text-sm">
                            <div>
                                ...
                                <span class="text-slate-800 font-semibold ml-2">{{ $category->created_at ? $category->created_at->format('d/m/Y à H:i') : '—' }}</span>
                            </div>
                            <div>
                                <span class="text-slate-500">Dernière modification :</span>
                                <span class="text-slate-800 font-semibold ml-2">{{ $category->updated_at ? $category->updated_at->format('d/m/Y à H:i') : '—' }}</span>
                            </div>
                        </div>
                    </div>
                </div>
