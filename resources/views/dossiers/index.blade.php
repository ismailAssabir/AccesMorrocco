<x-app-layout>
    <div class="p-8 bg-[#F8FAFC] min-h-screen">
        ...
        {{-- Filtres --}}
        <form method="GET" action="{{ route('dossiers.index') }}" class="mb-6 flex flex-col md:flex-row gap-3">
            ...
            @if(request('search') || request('status') || request('idDepartement'))
        ...
        <div class="bg-white border border-slate-200 rounded-3xl shadow-sm overflow-hidden">
            ...
            <table class="w-full text-sm table-fixed">
                ...
                <tbody class="divide-y divide-slate-50">
                    ...
                    <tr class="hover:bg-slate-50 transition-colors">
                        ...
                        <td class="px-4 py-4">
                            <p class="font-black text-slate-700 text-xs">{{ $dossier->reference }}</p>
                            <p class="text-slate-400 text-xs">{{ $dossier->nombreJours }} jour(s)</p>
                        </td>
                            ...
                            <div class="flex items-center gap-2">
                                <div class="w-7 h-7 rounded-lg bg-[#b11d40]/10 flex items-center justify-center flex-shrink-0">
                                    <span class="text-[#b11d40] font-black" style="font-size:9px">
                                        {{ strtoupper(substr($dossier->client->firstName ?? '?', 0, 1)) }}{{ strtoupper(substr($dossier->client->lastName ?? '?', 0, 1)) }}
                                    </span>
                                </div>
                        ...
                        <td class="px-4 py-4">
                            <p class="text-xs font-black text-slate-700">{{ number_format($dossier->montant, 2) }}</p>
                            <p class="text-xs text-slate-400">MAD</p>
                        </td>

                        <td class="px-4 py-4">
                            <span class="px-2 py-1 rounded-lg text-xs font-black uppercase {{ $statusColors[$dossier->status] ?? 'bg-slate-100 text-slate-500' }}">
            ...
            <div class="px-6 py-4 border-t border-slate-100 flex items-center justify-between">
                <p class="text-xs text-slate-400 font-semibold">
                    {{ $dossiers->firstItem() }}–{{ $dossiers->lastItem() }} sur {{ $dossiers->total() }} dossiers
                </p>
                <div class="flex gap-1">
                    ...
                    @else
                        <a href="{{ $dossiers->previousPageUrl() }}" class="px-3 py-1.5 rounded-lg text-xs font-bold text-slate-600 bg-white border border-slate-200 hover:bg-[#b11d40] hover:text-white hover:border-[#b11d40] transition-all">‹</a>
                    @endif
                    ...
                    @if($dossiers->hasMorePages())
                        <a href="{{ $dossiers->nextPageUrl() }}" class="px-3 py-1.5 rounded-lg text-xs font-bold text-slate-600 bg-white border border-slate-200 hover:bg-[#b11d40] hover:text-white hover:border-[#b11d40] transition-all">›</a>
                    @else
                        <span class="px-3 py-1.5 rounded-lg text-xs font-bold text-slate-300 bg-slate-50 cursor-not-allowed">›</span>
                    @endif
                </div>
