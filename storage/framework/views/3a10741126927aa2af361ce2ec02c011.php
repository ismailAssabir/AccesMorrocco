...
<div class="flex flex-col md:flex-row md:items-center gap-4 bg-slate-50 border border-slate-100 p-5 rounded-2xl hover:shadow-md transition-shadow relative group animate-fadeIn">
    ...
    <div class="flex-1 min-w-0">
        ...
        <p class="text-sm text-slate-500 mt-1 flex items-center gap-2">
            ...
            <?php if($reunion->lieu): ?>
                ...
                <span class="text-xs font-medium text-slate-400"><?php echo e($reunion->lieu); ?></span>
            <?php endif; ?>
        </p>
    </div>
    ...
    <div class="flex items-center gap-4 sm:ml-auto">
        <span class="<?php echo e($reunion->type_color); ?> font-bold px-3 py-1 rounded-lg text-[10px] uppercase tracking-wider"><?php echo e($reunion->type); ?></span>

        <div class="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
            <button type="button" onclick='viewReunionDetails(<?php echo e(json_encode($reunion)); ?>)' class="flex items-center gap-2 px-4 py-2 bg-slate-200/50 text-slate-600 rounded-xl hover:bg-slate-200 hover:text-slate-800 transition-all font-bold text-xs shadow-sm">
                <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                Voir
            </button>

            <?php if(auth()->user()->type !== 'employee'): ?>
            <button type="button" onclick='openEditModal(<?php echo e(json_encode($reunion)); ?>)' class="flex items-center gap-2 px-4 py-2 bg-blue-50 text-blue-600 rounded-xl hover:bg-blue-600 hover:text-white transition-all font-bold text-xs shadow-sm">
            ...
            <button type="button" onclick="confirmDeleteReunion(<?php echo e($reunion->idReunion); ?>)" class="flex items-center justify-center w-9 h-9 bg-red-50 text-red-600 rounded-xl hover:bg-red-600 hover:text-white transition-all shadow-sm">
            ...
            <?php endif; ?>
            <?php if($reunion->lien): ?>
            <a href="<?php echo e($reunion->lien); ?>" target="_blank" class="p-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors shadow-lg shadow-indigo-600/20" title="Rejoindre">
...
<?php endif; ?>
<?php /**PATH C:\Users\ABA SOLUTIONS\Desktop\PROJET STAGE Travel Agency\AccesMorrocco\resources\views/reunions/partials/cards.blade.php ENDPATH**/ ?>