                        ...
                        <div class="mt-10 pt-8 border-t border-slate-100">
                            <div class="flex items-center justify-between mb-6">
                                <div class="flex items-center gap-3">
                                    <div class="w-10 h-10 rounded-2xl bg-[#be2346]/10 flex items-center justify-center text-[#be2346]">
                                        <svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                                    </div>
                                    <div>
                                        <h4 class="text-sm font-black text-slate-800 uppercase tracking-widest">Prestation Finale Validée</h4>
                                        <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Version: <?php echo e($presentation->titre); ?></p>
                                    </div>
                                </div>
                                <a href="<?php echo e(route('clients.presentations.show', $presentation->idPresentation)); ?>" class="flex items-center gap-2 px-4 py-2 bg-slate-50 text-slate-600 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-[#be2346] hover:text-white transition-all group">
                                    Détails complets
                                    <svg class="w-3 h-3 transform group-hover:translate-x-0.5 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M9 5l7 7-7 7"/></svg>
                                </a>
                            </div>
                            ...
                            <div class="grid grid-cols-1 gap-3">
                                ...
                                <div class="flex items-center justify-between p-5 bg-white border border-slate-100 rounded-2xl hover:border-[#be2346]/30 hover:shadow-md hover:shadow-slate-100 transition-all group">
                                    <div class="flex items-center gap-4">
                                        <div class="w-12 h-12 rounded-xl bg-slate-50 flex flex-col items-center justify-center text-slate-400 group-hover:bg-[#be2346]/5 group-hover:text-[#be2346] transition-colors">
                                            <span class="text-[10px] font-black uppercase leading-none mb-0.5">Qte</span>
                                            <span class="text-sm font-black leading-none"><?php echo e($item->quantity); ?></span>
                                        </div>
                                        <div>
                                            <p class="text-sm font-bold text-slate-800 leading-tight mb-1"><?php echo e($item->nom); ?></p>
                                            <div class="flex items-center gap-2">
                                                <span class="px-2 py-0.5 rounded-md bg-slate-100 text-[9px] font-black text-slate-500 uppercase tracking-tighter"><?php echo e($item->category->nom ?? 'Prestation'); ?></span>
                                                <span class="text-[10px] text-slate-400 font-medium"><?php echo e(number_format($item->prixUnitaire, 2, ',', ' ')); ?> MAD / unité</span>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="text-right">
                                        <p class="text-sm font-black text-slate-900"><?php echo e(number_format($item->totale, 2, ',', ' ')); ?> <span class="text-[10px] text-slate-400 font-light">MAD</span></p>
                                    </div>
                                </div>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </div>

                            <div class="mt-6 flex items-center justify-between p-6 bg-slate-900 rounded-[2rem] text-white overflow-hidden relative group">
                                <div class="relative z-10">
                                    <p class="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400 mb-1">Total de la proposition</p>
                                    <p class="text-2xl font-black"><?php echo e(number_format($presentation->total, 2, ',', ' ')); ?> <span class="text-xs font-light text-slate-500">MAD</span></p>
                                </div>
                                <div class="relative z-10 hidden sm:block">
                                    <div class="px-4 py-2 bg-white/10 rounded-xl border border-white/10 backdrop-blur-sm">
                                        <p class="text-[10px] font-black uppercase tracking-widest text-white/80">Statut: Accepté</p>
                                    </div>
                                </div>
                                
                                <div class="absolute -right-4 -bottom-4 w-24 h-24 bg-[#be2346]/20 rounded-full blur-2xl group-hover:bg-[#be2346]/40 transition-all"></div>
                            </div>
                        </div>
                        <?php endif; ?>
                    </div>
...
<?php echo $__env->make('layouts.client', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\dell\Desktop\AccesMorrocco\resources\views/clients/dossiers/show.blade.php ENDPATH**/ ?>