        ...
        <div class="max-w-4xl mx-auto">
            ...
            <div class="flex items-center justify-between mb-8">
                <div>
                    ...
                    <p class="text-slate-500 text-sm mt-1 font-medium">Référence: #REC-{{ str_pad($Reclamation->idReclamation, 5, '0', STR_PAD_LEFT) }}</p>
                </div>
                <a href="{{ url('/reclamations') }}" class="flex items-center gap-2 text-slate-500 hover:text-slate-800 transition-colors font-bold text-sm">
                ...
                <div class="md:col-span-2 space-y-6">
                    <div class="bg-white border border-slate-200 rounded-[32px] shadow-sm p-8">
                        <div class="flex items-center justify-between mb-6">
                            <h2 class="text-xl font-black text-slate-800">{{ $Reclamation->titre }}</h2>
                            <span class="px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-wider {{ $Reclamation->status == 'resolue' ? 'bg-emerald-100 text-emerald-600' : ($Reclamation->status == 'en_cours' ? 'bg-amber-100 text-amber-600' : 'bg-blue-100 text-blue-600') }}">
                    ...
                    <div class="bg-white border border-slate-200 rounded-[32px] shadow-sm p-8">
                        ...
                        <div class="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100">
                            ...
                            <a href="{{ asset('storage/' . $Reclamation->fichier) }}" target="_blank" class="px-4 py-2 bg-white border border-slate-200 rounded-lg text-xs font-black text-slate-600 hover:bg-slate-50 transition-colors">
                                Ouvrir
                            </a>
                        </div>
                    </div>
                        ...
                        <div class="space-y-6">
                            <div>
                                ...
                                <div class="flex items-center gap-2">
                                    <div class="w-2 h-2 rounded-full {{ $Reclamation->priorite == 'haute' ? 'bg-red-500' : ($Reclamation->priorite == 'moyenne' ? 'bg-amber-500' : 'bg-blue-500') }}"></div>
                                    <span class="text-sm font-bold text-slate-700 capitalize">{{ $Reclamation->priorite }}</span>
                                </div>
                            </div>

                            <div>
                                <p class="text-[10px] font-black uppercase text-slate-400 mb-1">Date d'envoi</p>
                                <p class="text-sm font-bold text-slate-700">{{ $Reclamation->created_at->format('d M Y, H:i') }}</p>
                            </div>
                            ...
                            <div class="pt-6 border-t border-slate-100">
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p class="text-xs font-bold text-slate-700">{{ $Reclamation->user->firstName }} {{ $Reclamation->user->lastName }}</p>
                                        <p class="text-[10px] text-slate-400">{{ $Reclamation->user->post ?? 'Employé' }}</p>
                                    </div>
                                </div>
                            </div>
                            @endif
                        ...
                        <div class="space-y-3">
                            @if(auth()->user()->type === 'admin' || auth()->user()->type === 'manager')
                                <button onclick="toggleModal('replyReclamationModal')" class="w-full py-3 bg-[#be2346] text-white rounded-xl text-xs font-black hover:bg-[#a01d3a] transition-all">
                                    {{ $Reclamation->reponse ? 'Modifier la réponse' : 'Répondre' }}
                                </button>
                            @endif
            ...
            <div class="bg-white w-full max-w-md rounded-[32px] shadow-2xl p-8 relative overflow-hidden">
                ...
                <div class="flex flex-col items-center text-center">
                    ...
                    <div class="flex w-full gap-3">
                        ...
                        <form action="/reclamation/delete/{{ $Reclamation->idReclamation }}" method="POST" class="flex-1">
                            @csrf
                            @method('DELETE')
    ...
    <div id="replyReclamationModal" class="fixed inset-0 z-[100] hidden overflow-y-auto">
        ...
        <div class="relative flex items-center justify-center min-h-screen p-4">
            <div class="bg-white w-full max-w-lg rounded-[32px] shadow-2xl p-8 relative">
                ...
                <form action="/reclamation/reponse/{{ $Reclamation->idReclamation }}" method="POST" class="space-y-4">
                    @csrf
                    @method('PATCH')
                    
                    <div>
                        <label class="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Votre réponse</label>
                        <textarea name="reponse" rows="6" required placeholder="Tapez votre réponse ici..." class="w-full mt-1 bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-[#be2346]/20 outline-none transition-all">{{ $Reclamation->reponse }}</textarea>
                    </div>
