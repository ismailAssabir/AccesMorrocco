...
<?php endif; ?>
                ...
                <div class="flex flex-wrap items-center justify-center gap-3 w-full mt-4 pt-4 border-t border-slate-100">
                    ...
                    <div class="relative">
                        <select name="period" id="periodSelect" onchange="toggleCustomDates()"
                            ...
                            <option value="month" <?php echo e(request('period') == 'month' ? 'selected' : ''); ?>>Ce mois-ci</option>
                    ...
                    <div class="relative">
                        <select name="has_justification"
                            ...
                            <option value="yes" <?php echo e(request('has_justification') == 'yes' ? 'selected' : ''); ?>>Avec Justificatif</option>
                            <option value="no" <?php echo e(request('has_justification') == 'no' ? 'selected' : ''); ?>>Sans Justificatif</option>
                        </select>
                        <div class="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
                            <svg class="h-3 w-3 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path d="M19 9l-7 7-7-7" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" />
        ...
        <div id="tableContainer"
                    ...
                    <tbody class="divide-y divide-slate-100" id="pointage-tbody">
                        <?php $__empty_1 = true; $__currentLoopData = $pointages; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $pointage): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            ...
                            <tr class="hover:bg-slate-50 transition-colors pointage-row"
                                ...
                                data-role="<?php echo e(strtolower($user->type ?? 'employee')); ?>" data-date="<?php echo e($pointage->date); ?>"
                                        ...
                                        <div>
                                            ...
                                            <?php if($user): ?>
                                                <div class="flex flex-col mt-0.5">
                                                    <p class="text-[10px] font-bold <?php echo e(($user->type ?? '') === 'admin' ? 'text-[#be2346]' : (($user->type ?? '') === 'manager' ? 'text-indigo-500' : 'text-slate-400')); ?>">
                                                        <?php echo e(ucfirst($user->type ?? 'User')); ?>

                                                    </p>
                                                    <p class="text-[9px] text-slate-400 font-medium"><?php echo e($user->email ?? ''); ?></p>
                                                </div>
                                            <?php endif; ?>
                                        </div>
                                    </div>
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex flex-col">
                                        ...
                                        <?php if($delayIn): ?>
                                            <span
                                                class="text-[9px] font-black text-amber-500 mt-1 ml-1 uppercase tracking-tighter"><?php echo e($delayIn); ?></span>
                                        <?php endif; ?>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex flex-col">
                                        <?php if($pointage->heureSortie): ?>
                                            <span
                                                class="font-mono text-xs bg-slate-100 text-slate-600 px-2.5 py-1 rounded-lg font-bold w-fit">
                                                <?php echo e(\Carbon\Carbon::parse($pointage->heureSortie)->format('H:i')); ?>
                                ...
                                <td class="px-6 py-4 text-xs text-slate-400 font-mono">
                                    <?php if($pointage->gps): ?>
                                        <a href="https://www.google.com/maps/search/?api=1&query=<?php echo e($pointage->gps); ?>"
                                            target="_blank"
                                ...
                                <td class="px-6 py-4 max-w-xs">
                                    <?php if($pointage->justification): ?>
                                        <button type="button" onclick="openAdminJustificationModal(this)"
                                            data-id="<?php echo e($pointage->idPointage); ?>"
                                            data-justification="<?php echo e($pointage->justification); ?>"
                                            data-type="<?php echo e($pointage->typejustif); ?>"
                                            data-status="<?php echo e($pointage->justification_status); ?>"
                                            data-file="<?php echo e($pointage->fichier ? Storage::url($pointage->fichier) : ''); ?>"
                                            data-refus="<?php echo e($pointage->motif_refus); ?>"
            ...
            <?php if(method_exists($pointages, 'links') && $pointages->hasPages()): ?>
                <div class="px-6 py-4 border-t border-slate-100 bg-white">
                    <?php echo e($pointages->links('vendor.pagination.tailwind_saas')); ?>
            ...
            <form action="<?php echo e(route('admin.settings.update')); ?>" method="POST" class="p-7 space-y-5">
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="companyGps" placeholder="Ex: 32.9348,-6.0234"
                        value="<?php echo e($settings->companyGps ?? ''); ?>"
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-1.5">
                        ...
                        <input type="number" name="distance" placeholder="Ex: 200" min="10" max="5000"
                            value="<?php echo e($settings->distance ?? ''); ?>"
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input type="number" name="maxDelay" placeholder="Ex: 15" min="0" max="120"
                            value="<?php echo e($settings->maxDelay ?? ''); ?>"
                            class="w-full bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3 text-sm outline-none transition-all focus:border-[#be2346] focus:ring-4 focus:ring-[#be2346]/5">
                    </div>
                </div>
...
<?php endif; ?><?php /**PATH C:\Users\ABA SOLUTIONS\Desktop\PROJET STAGE Travel Agency\AccesMorrocco\resources\views/Adminpointage.blade.php ENDPATH**/ ?>