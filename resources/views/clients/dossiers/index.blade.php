{{-- resources/views/clients/dossiers/index.blade.php --}}

@extends('layouts.client')

@section('title', 'Les Dossiers — Espace Client')
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-5">
                ...
                <div>
                    ...
                    <p class="text-2xl font-black text-slate-800">{{ $dossiers->total() }}</p>
                </div>
            </div>
            ...
            <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-5">
                ...
                <div>
                    ...
                    <p class="text-2xl font-black text-slate-800">{{ number_format($dossiers->sum('montant'), 2, ',', ' ') }} MAD</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
            <div class="px-6 py-5 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                ...
                <span class="px-3 py-1 bg-white border border-slate-200 rounded-full text-[10px] font-bold text-slate-500">
                    {{ $dossiers->count() }} dossier(s) sur cette page
                </span>
            </div>
                                ...
                                <tr class="hover:bg-slate-50/80 transition-colors group">
                                    <td class="px-6 py-4">
                                        <div class="flex flex-col">
                                            <span class="text-sm font-black text-slate-700">#{{ $dossier->reference }}</span>
                                            <span class="text-[10px] text-slate-400 italic">Créé le {{ $dossier->created_at->format('d/m/Y') }}</span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4">
                                        <p class="text-sm font-bold text-slate-700 group-hover:text-[#be2346] transition-colors">
                                            {{ $dossier->distination ?? 'Non spécifiée' }}
                                        </p>
                                        <p class="text-[10px] text-slate-500 font-medium">
                                            @if($dossier->dateVoyage)
                                                Départ le {{ \Carbon\Carbon::parse($dossier->dateVoyage)->format('d/m/Y') }} 
                                                ({{ $dossier->nombreJours }} jours)
                                            @else
                                                Date non fixée
                                            @endif
                                        </p>
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        ...
                                        @endphp
                                        <span class="inline-flex items-center px-3 py-1 rounded-full text-[10px] font-black uppercase border {{ $statusClasses }}">
                                            {{ $statusLabel }}
                                        </span>
                                    </td>
