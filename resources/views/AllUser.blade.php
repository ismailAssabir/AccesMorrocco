<x-app-layout>
                        ...
                        <tr class="hover:bg-slate-50/80 transition-colors group">
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('users.show', $user->idUser) }}" onclick="event.preventDefault();" class="transition-all" title="Voir les détails">
                                        <button type="button" onclick='openViewModal(@json($user))' 
                                    ...
                                    <a href="{{ route('users.edit', $user->idUser) }}" onclick="event.preventDefault();" class="transition-all" title="Modifier">
    ...
    <script>
        ...
        function openEditModal(user, skipPush = false) {
            ...
            // Construct the update URL using the named route
            const updateUrlPattern = "{{ route('users.update', ':id') }}";
            document.getElementById('editForm').action = updateUrlPattern.replace(':id', (user.idUser || user.id || user.id_user));
