        ...
        <table style="width: 100%;">
            <tr>
                ...
                <td width="34%" style="text-align: center; vertical-align: middle;">
                    ...
                    <div style="font-size: 9px; color: #94a3b8; margin-top: 4px;">Période du {{ now()->format('d/m/Y') }}</div>
                </td>
    ...
    <table class="stats-table">
        <tr>
            <td class="stat-card bg-main" width="33%">
                ...
                <div class="stat-card-value">{{ $dossiers->count() }}</div>
            </td>
            <td class="stat-card bg-money" width="33%">
                <div class="stat-card-title">Chiffre d'Affaires</div>
                <div class="stat-card-value">{{ number_format($dossiers->sum('montant'), 0, ',', ' ') }} <small style="font-size: 10px;">MAD</small></div>
            </td>
            <td class="stat-card bg-open" width="33%">
                <div class="stat-card-title">Dossiers ouverts</div>
                <div class="stat-card-value">{{ $dossiers->where('status', 'ouvert')->count() }}</div>
            </td>
        </tr>
    </table>
    ...
    <table class="data-table">
        ...
        <tbody>
            @forelse($dossiers as $d)
                <tr>
                    <td style="font-weight: bold; color: #1e293b;">{{ $d->reference }}</td>
                    <td>
                        <div class="client-name">{{ $d->client->firstName ?? '—' }} {{ $d->client->lastName ?? '' }}</div>
                        <div class="dest-text">{{ $d->distination ?? 'Destination non spécifiée' }}</div>
                    </td>
                    <td style="color: #4b5563;">{{ $d->departement->title ?? '—' }}</td>
                    <td style="text-align: center;">{{ $d->nombrePersonnes }}</td>
                    <td style="text-align: center;">{{ $d->nombreJours }}</td>
                    <td style="text-align: center;" class="amount">{{ number_format($d->montant, 2, ',', ' ') }} MAD</td>
                    <td style="text-align: center; color: #1e293b;">
                        {{ $d->dateVoyage ? \Carbon\Carbon::parse($d->dateVoyage)->format('d/m/Y') : '—' }}
                    </td>
                    <td style="text-align: center;">
                        <div class="badge badge-{{ strtolower($d->status) }}">
                            {{ $d->status }}
                        </div>
                    </td>
                </tr>
